	...
	case Common::KEYCODE_c:
		if (state.flags & Common::KBD_CTRL) {
			if (!getEditString().empty()) {
				...
				//warning("Selected: %d %d %d %d %s", selBegin, selEnd, _selOffset, _selCaretPos, selected.encode().c_str());
	...
	if (isCharAllowed(state.ascii)) {
		...
		if (_selCaretPos >= 0) {
			...
			if(_editString.size() > 0)
				selBegin++;
			warning("setting caretPos to: %d and string size: %d", caretVisualPos(selBegin), _editString.size());
			setCaretPos(caretVisualPos(selBegin));
			_selCaretPos = -1;
			_selOffset = 0;
		} else {
			...
			const int logicalPosition = caretLogicalPos();
			warning("Inserting char at: %d and string size: %d", logicalPosition + 1, _editString.size());
