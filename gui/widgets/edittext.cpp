...
void EditTextWidget::drawWidget() {
	...
	for (int i = 0; i < parts.size(); i++) {
		...
		int clipW = MIN(partW, (int)drawRect.width());
		_inversion = i == 1 ? ThemeEngine::kTextInversionFocus : ThemeEngine::kTextInversionNone;
		int clipR = MIN(x + clipW, (int)drawRect.right);
		int clipL = MAX(x, (int)drawRect.left);
		warning("x: %d drawRect.left: %d", x, drawRect.left);
		warning("Part[%d]: Drawing b/w x: %d x2: %d scrollOffset %d width: %d", i, clipL, clipR, scrollOffset, partW);
		g_gui.theme()->drawText(Common::Rect(clipL, y, clipR, y + drawRect.height()), part, _state,
            _drawAlign, _inversion, -scrollOffset, false, _font, ThemeEngine::kFontColorNormal, true,
            _textDrawableArea);
		x += partW;
		x -= scrollOffset;
		scrollOffset = partW > scrollOffset ? 0 : scrollOffset - partW; 
	}
}
